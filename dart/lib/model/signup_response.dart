...
class SignupResponse {
  ...
  @override
  String toString() => 'SignupResponse[errored=$errored, messages=$messages, token=$token, refreshToken=$refreshToken, username=$username, displayName=$displayName, email=$email, roles=$roles, emailVerified=$emailVerified, lockedAccount=$lockedAccount, deletedAccount=$deletedAccount]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "SignupResponse[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "SignupResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());

...
class OtpRequestResponse {
  ...
  /// Returns a new [OtpRequestResponse] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static OtpRequestResponse? fromJson(dynamic value) {
    if (value is Map) {
  ...
  static List<OtpRequestResponse> listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = OtpRequestResponse.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, OtpRequestResponse> mapFromJson(dynamic json) {
    final map = <String, OtpRequestResponse>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = OtpRequestResponse.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
